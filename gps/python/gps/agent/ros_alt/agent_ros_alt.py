""" This file defines an alternative agent for the KUKA LWR ROS environment.
    Copied from the PR2 ROS agent. """
...
from gps.agent.ros_alt.ros_alt_utils import ServiceEmulator, msg_to_sample, \
...
class AgentROSAlt(Agent):
    ...
    def __init__(self, hyperparams, init_node=True):
        ...
        if init_node:
            rospy.init_node('gps_agent_ros_alt_node')
    ...
    def get_data(self, arm=TRIAL_ARM):
        ...
        Args:
            arm: TRIAL_ARM or AUXILIARY_ARM. AUXILIARY_ARM fails.
        """
        if arm == AUXILIARY_ARM:
            raise Exception('AgentROSAlt tried to get data from bad arm')
    ...
    def relax_arm(self, arm):
        ...
        Args:
            arm: Either TRIAL_ARM or AUXILIARY_ARM. AUXILIARY_ARM fails.
        """
        if arm == AUXILIARY_ARM:
            raise Exception('AgentROSAlt tried to relax bad arm')
