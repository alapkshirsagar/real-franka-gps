...
class GPSTrainingGUI(object):
    ...
    def __init__(self, hyperparams):
        # self.my_log = open("/home/walkingbeard/mylog.txt", "w+")
    ...
    def update(self, itr, algorithm, agent, traj_sample_lists, pol_sample_lists):
        ...
        # print [algorithm.prev[m].cs for m in range(algorithm.M)]
        # for m in range(algorithm.M):
        #     s = str(m) + "\n  cs: " + str(algorithm.prev[m].cs) + "\n"
        #     self.my_log.write(s)
        #     s = "  sum in ax1: " + str(np.sum(algorithm.prev[m].cs, axis = 1)) + "\n"
        #     self.my_log.write(s)
        #     s = "  mean: " + str(np.mean(np.sum(algorithm.prev[m].cs))) + "\n"
        #     self.my_log.write(s)
