...
void RobotPlugin::initialize_ros(ros::NodeHandle& n)
{
  ...
  // Create subscribers.
  position_subscriber_ = n.subscribe("/gps_controller_position_command", 1,
                                     &RobotPlugin::position_subscriber_callback, this);
  trial_subscriber_ = n.subscribe("/gps_controller_trial_command", 1,
                                  &RobotPlugin::trial_subscriber_callback, this);
  test_sub_ = n.subscribe("/test_sub", 1, &RobotPlugin::test_callback, this);
  relax_subscriber_ = n.subscribe("/gps_controller_relax_command", 1,
  ...
  action_subscriber_tf_ = n.subscribe("/gps_controller_sent_robot_action_tf", 1,
                                      &RobotPlugin::tf_robot_action_command_callback, this);
  tf_publisher_.reset(new
                      realtime_tools::RealtimePublisher<gps_agent_pkg::TfObsData>(n, "/gps_obs_tf",
                          1));
}
...
void RobotPlugin::configure_sensors(OptionsMap &opts)
{
  ...
  current_time_step_sample_->set_meta_data(
    gps::ACTION, active_arm_torques_.size(), SampleDataFormatEigenVector,
    sample_metadata);
...
void RobotPlugin::update_sensors(ros::Time current_time,
                                 bool is_controller_step)
{
  // ROS_INFO("    Entering rp::update_sensors()");
  // ROS_INFO_STREAM("    Trial sensor size: " << sensors_.size());
  // ROS_INFO_STREAM("    Passive sensor size: " << sensors_.size());
  if (!sensors_initialized_)
    return; // Don't try to use sensors until initialization finishes.

  // ROS_INFO_STREAM("-----Updating active sensors\n");
  ...
  // ROS_INFO_STREAM("-----Updating passive sensors\n");
  ...
  if (aux_data_request_waiting_)
    ...
    // ROS_INFO("    Exiting rp::update_sensors()");
}

// Update the controllers at each time step.
void RobotPlugin::update_controllers(ros::Time current_time,
                                     bool is_controller_step)
{
  // Update passive arm controller.
  // TODO - don't pass in wrong sample if used
  passive_arm_controller_->update(this, current_time, current_time_step_sample_,
                                  passive_arm_torques_);

  bool trial_init = trial_controller_ != NULL
                    && trial_controller_->is_configured() && controller_initialized_;
  ...
  if (trial_init) trial_controller_->update(this, current_time,
        current_time_step_sample_, active_arm_torques_);
  else active_arm_controller_->update(this, current_time,
                                        current_time_step_sample_, active_arm_torques_);
...
void RobotPlugin::trial_subscriber_callback(const
    gps_agent_pkg::TrialCommand::ConstPtr& msg)
{
  ...
  if (msg->controller.controller_to_execute == gps::LIN_GAUSS_CONTROLLER)
    {
      std::cout << __FILE__ << ": standard LGC\n";
      // static bool togged(false);
      // if (!togged)
      //   {
      //     std::cout << __FILE__ << ": " << msg->controller.lingauss << std::endl;
      //     togged = true;
      //   }
    ...
    {
      std::cout << __FILE__ << ": Caffe controller\n";
  ...
  if ( msg->ee_points.size() % 3 != 0)
    {
      ROS_ERROR("Got %d ee_points (must be multiple of 3)",
                (int)msg->ee_points.size());
    }
...
void RobotPlugin::relax_subscriber_callback(const
...
void RobotPlugin::data_request_subscriber_callback(const
    gps_agent_pkg::DataRequest::ConstPtr& msg)
{
...
void RobotPlugin::get_fk_solver(boost::shared_ptr<KDL::ChainFkSolverPos>
                                &fk_solver, boost::shared_ptr<KDL::ChainJntToJacSolver> &jac_solver,
...
void RobotPlugin::tf_robot_action_command_callback(const
    gps_agent_pkg::TfActionCommand::ConstPtr& msg)
{
