...
void ROSTopicSensor::update_data_vector(const std_msgs::Float64MultiArray::ConstPtr& msg) {
    ...
    for (int i = 0; i < data_size_; i++)
	{
        ...
        difference_data_[i] = latest_data_[i] - previous_data_[i];
...
void ROSTopicSensor::set_sample_data_format(boost::scoped_ptr<Sample>& sample) 
{
    ...
    sample->set_meta_data(gps::END_EFFECTOR_POINTS,data_size_,SampleDataFormatEigenVector,data_metadata);
    sample->set_meta_data(gps::END_EFFECTOR_POINT_VELOCITIES,data_size_,SampleDataFormatEigenVector,data_metadata);
