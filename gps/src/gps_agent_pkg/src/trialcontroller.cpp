...
void TrialController::update(RobotPlugin *plugin, ros::Time current_time,
    boost::scoped_ptr<Sample>& sample, Eigen::VectorXd &torques)
{
    if (is_finished()){
    ...
    // publish the observation for consumption. Can be implemented in subclass
    // if you want the observations published to a ros node. Used for async
    // controllers like the tf_controller.
    ...
    sample->set_data(step_counter_,
                     gps::ACTION,torques,torques.size(),
